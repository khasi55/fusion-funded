...
export default function CompetitionDetailsClient({ competitionId }: { competitionId: string }) {
    ...
    const fetchUserTrades = async (challengeId: string, username: string) => {
        ...
        try {
            const backendUrl = typeof window !== 'undefined' ? "" : (process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001');
            const response = await fetch(`${backendUrl}/api/competitions/trades/${challengeId}`);
            if (response.ok) {
    ...
    const handleJoin = async () => {
        ...
        try {
            ...
            if (competition.entry_fee && competition.entry_fee > 0) {
                ...
                const backendUrl = typeof window !== 'undefined' ? "" : (process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:3001');
                const response = await fetch(`${backendUrl}/api/competitions/${competitionId}/join`, {
                    method: 'POST',
                    headers: {

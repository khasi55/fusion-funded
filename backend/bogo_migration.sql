...
BEGIN
    ...
    -- Ensure columns exist (handling potential schema drift)
    ALTER TABLE public.discount_coupons ADD COLUMN IF NOT EXISTS uses_count INTEGER DEFAULT 0;
    ALTER TABLE public.discount_coupons ADD COLUMN IF NOT EXISTS affiliate_id UUID REFERENCES auth.users(id);
    ALTER TABLE public.discount_coupons ADD COLUMN IF NOT EXISTS commission_rate NUMERIC;
    ALTER TABLE public.discount_coupons ADD COLUMN IF NOT EXISTS max_discount_amount NUMERIC;
